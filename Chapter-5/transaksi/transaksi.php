...
?>
	...
	?>
			...
			<div class="col-lg-12">
				<div class="form-panel" style="width:50%;">
					...
					<form action="transaksi/proses_transaksi.php?pros=angsur" method="post" id="form" name="frmAdd">
						...
						<?php
						$kk = mysql_fetch_array(mysql_query("SELECT * from t_pinjam where kode_pinjam='$kodep' and kode_anggota='$kode'"));
						$tempo = $kk['tgl_tempo'];
						$dat = date("Y-m-d");
						if ($dat > $tempo) {
		...
		<?php
				...
				$kode = $_GET['kode_anggota'];
				$anggota = mysql_fetch_array(mysql_query("SELECT nama_anggota from t_anggota where kode_anggota='$kode'")); ?>
		<div class="row mt">
			<div class="col-lg-12">
				<div class="form-panel">
					<h4 class="mb"><span class='glyphicon glyphicon-briefcase'></span> Transaksi <?php echo $anggota['nama_anggota']; ?>
						...
						<span style="float:right;">
							<?php $df = mysql_fetch_array(mysql_query("SELECT * FROM t_pinjam where kode_anggota='$kode' order by kode_pinjam desc"));
						...
						<span style="float:right;">
							...
							while ($verida = mysql_fetch_array($rino)) {
								if ($verida['nama_simpanan'] == 'wajib') {
									$baru = mysql_fetch_array(mysql_query("SELECT *FROM t_simpan where kode_anggota='$kode' and jenis_simpan='wajib' order by kode_simpan desc "));
									$numrow = mysql_num_rows($baru);
									$data = $baru['tgl_mulai'];
									$now = date("Y-m-d");
									if ($data == $now) {
